...
2. ``openvtb-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

**develop** branch
------------------

1. ``openvtb-django-develop``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``openvtb-django-develop-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

pull requests
-------------
1. ``openvtb-pr-django``: backend tests, runs ``./bin/jenkins_django_pr.sh``.
2. ``openvtb-pr-js``: frontend tests, runs ``./bin/jenkins_js.sh``.
3. ``openvtb-pr-sass``: checks that sass compiles, runs ``./bin/jenkins_sass.sh``.
4. ``openvtb-pr-isort``: checks that imports are correctly
