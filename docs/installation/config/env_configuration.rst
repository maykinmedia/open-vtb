...
* ``DB_NAME``: name of the PostgreSQL database. Defaults to: ``openvtb``.
* ``DB_USER``: username of the database user. Defaults to: ``openvtb``.
* ``DB_PASSWORD``: password of the database user. Defaults to: ``openvtb``.
...
* ``ELASTIC_APM_SERVICE_NAME``: Name of the service for this application in Elastic APM. Defaults to ``openvtb - <environment>``.
...
* ``DEFAULT_FROM_EMAIL``: The default email address from which emails are sent. Defaults to: ``openvtb@example.com``.
...
* ``EVENTS_BERICHTEN_JOB_MINUTE``: Minute of execution (0 - 59). The job is triggered at this minute within each scheduled hour interval, as defined by the hour interval configuration. The schedule is evaluated in UTC timezone. Defaults to: ``0``.
* ``EVENTS_BERICHTEN_JOB_HOUR``: Hour interval (1 - 23). Determines the frequency of execution in hours. The job runs repeatedly based on this interval rather than at a single fixed hour. Default is every hour. The schedule is evaluated in UTC timezone. Defaults to: ``1``.
* ``EVENTS_TAKEN_JOB_MINUTE``: Minute of execution (0 - 59). The job is triggered at this minute within each scheduled hour interval, as defined by the hour interval configuration. The schedule is evaluated in UTC timezone. Defaults to: ``0``.
* ``EVENTS_TAKEN_JOB_HOUR``: Hour interval (1 - 23). Determines the frequency of execution in hours. The job runs repeatedly based on this interval rather than at a single fixed hour. Default is every hour. The schedule is evaluated in UTC timezone. Defaults to: ``1``.
