...
from openvtb.accounts.tests.factories import TokenFactory, UserFactory
...
class CorsMixin:
    def setUp(self):
        super().setUp()
        mocker = patch(
            "openvtb.utils.middleware.get_version_mapping",
            return_value={"/": "1.0.0"},
        )
        mocker.start()
...
@override_settings(ROOT_URLCONF="openvtb.tests.test_cors_configuration")
class DefaultCORSConfigurationTests(CorsMixin, APITestCase):
    ...
    def test_credentialed_request(self):
        user = UserFactory.create(superuser=True, password="secret")
...
@override_settings(
    ROOT_URLCONF="openvtb.tests.test_cors_configuration",
...
class CORSEnabledWithAuthHeaderTests(CorsMixin, APITestCase):
    ...
    def test_credentialed_request(self):
        TokenFactory.create(key="foobarbaz")
        response = self.client.get(
            "/cors",
