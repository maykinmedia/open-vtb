...
class TestApiOidcAuthentication(OIDCMixin, VCRMixin, TestCase):
    ...
    def test_get(self):
        ...
        with self.subTest("verzoek"):
            ...
            response = self.client.get(
                reverse(
                    "verzoeken:verzoek-detail", kwargs={"uuid": str(self.verzoek.uuid)}
                ),
                headers={"Authorization": f"Bearer {token}"},
            )
            self.assertEqual(response.status_code, HTTP_200_OK)
            self.assertEqual(response.data["uuid"], str(self.verzoek.uuid))

        with self.subTest("verzoektype"):
            # get list
            response = self.client.get(
                reverse("verzoeken:verzoektype-list"),
                headers={"Authorization": f"Bearer {token}"},
            )
            self.assertEqual(User.objects.all().count(), 1)
            self.assertEqual(response.status_code, HTTP_200_OK)
            ...
            response = self.client.get(
                reverse(
                    ...
                    kwargs={"uuid": str(self.verzoektype.uuid)},
                ),
                headers={"Authorization": f"Bearer {token}"},
            )
            self.assertEqual(response.status_code, HTTP_200_OK)
        ...
        with self.subTest("verzoek-version"):
            ...
            # get list
            response = self.client.get(
                f"http://testserver{(reverse('verzoeken:verzoektypeversion-list', kwargs={'verzoektype_uuid': str(self.verzoektype.uuid)}))}",
                headers={"Authorization": f"Bearer {token}"},
            )
            self.assertEqual(response.status_code, HTTP_200_OK)
